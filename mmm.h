...
typedef struct {
    mmm_memblock *free_blocks;
    mmm_memblock *used_blocks;
...
mmm_pool mmm_pool_init(uint64_t size, uint64_t alignment, uint64_t managed_ptrs_per_block);
mmm_pool mmm_pool_init_preallocated(uint64_t size, uint64_t alignment, uint64_t managed_ptrs_per_block, void *mem);
void mmm_pool_grow(mmm_pool pool, uint64_t new_size);
void mmm_pool_purge(mmm_pool pool, uint32_t (*decider_func)(mmm_memblock *p_block));
void mmm_pool_uninit(mmm_pool pool);
...
void mmm_register_managed_ptr(void *allocation, void **p_ptr);
